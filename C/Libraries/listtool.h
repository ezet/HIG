...
#if !defined(__LISTTOOL_H)             //  Ensures that this file will be
#define __LISTTOOL_H                   //    included only ONCE in each file.
...
class Element  {                       //  Base class 'Element'.
  ...
  public:
    Element()        {  element_type = 'B';    }   //  Sets element type.
    virtual ~Element()  {     }                    //  Virtual, empty destr.
    char get_type()  {  return  element_type;  }   //  Return element type.
  ...
  public:
    ...
    void display_list();                //      X          X           X
			     ...
			     //  DEFINITION OF MEMBERFUNCTIONS:
				       //  Searches in a sorted list for the
				       //    position for insertion of the
				       //    element pointed to by 'eptr'.
				       //    RETURNS A POINTER TO THE NODE
				       //    BEFORE THE RELEVANT POSITION.
...
bool List::is_empty()  {               //  Returns if list is empty or not.
  return  (no_in_list == 0)  ?  true  :  false; 
}
