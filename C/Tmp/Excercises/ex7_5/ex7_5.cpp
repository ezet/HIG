...
public:
	...
	void printFrac() { cout << num << '/' << denom << endl; }
	void fadd(fraction, fraction);
	void fsub(fraction, fraction);
	void fmul(fraction, fraction);
	void fdiv(fraction, fraction);
	void fsum(fraction, fraction);
	void favg(fraction, int);
};

void fraction::fsum(fraction f1, fraction f2) {
	num = f1.num + f2.num;
	denom = f1.denom + f2.denom;
}

void fraction::favg(fraction f1, int div) {
	num = f1.num / div;
	denom = f1.denom/ div;
}
...
int main() {
	fraction f1, f2, res, sum, average;
	...
	for( int i = 0; i < MAX; i++ ) {
		fraction[i].getFrac();
		sum.fsum(sum, fraction[i]);
	}

	average.favg(sum, MAX);
	average.lowterms();
	average.printFrac();
	...
	res.calcFrac(f1, f2);
	res.lowterms();
	cout << "The result is: "; res.printFrac();
